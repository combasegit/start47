...
//メタ情報
$PAGE_TITLE=$school_name."カリキュラム・コース";
$PAGE_DESCRIPTION=$school_name."のカリキュラム・コースについてご案内いたします。";
$PAGE_KEYWORDS=$school_name.",カリキュラム,コース,通信制高校";

while($c_data = $c_res->fetch_assoc()){$z++;
	$curriculum_title[$z]=$c_data['curriculum_title'];
	$curriculum_txt[$z]=$c_data['curriculum_txt'];
}
...
?>
	<main>
		<div class="bread_list">
			<a href="/">ホーム</a>&nbsp;&gt;&nbsp;<a href="/search/">学校を探す</a>&nbsp;&gt;&nbsp;<a href="/search/<?=$AREA_E[$THIS_SCHOOL_AREA]?>/"><?=$AREA[$THIS_SCHOOL_AREA]?></a>&nbsp;&gt;&nbsp;<a href="/search/<?=$AREA_E[$THIS_SCHOOL_AREA]?>/<?=$PREFECTURE_E[$THIS_SCHOOL_PREF]?>/"><?=$PREFECTURE[$THIS_SCHOOL_PREF]?></a>&nbsp;&gt;&nbsp;<a href="./"><?=$school_name?></a>&nbsp;&gt;&nbsp;カリキュラム・コース
		</div>
	<?if($err==1){?>
		<article id="main_box">
...
?>
			<article id="detail_box">
				<h3>カリキュラム・コース</h3>
				<div>
					<?for($i=1;$i<=$max_curric_cnt;$i++){if(strlen($curriculum_title[$i])){?>
					<article class="curriculum">
						<h4><?=$curriculum_title[$i]?></h4>
						<p><?=nl2br($curriculum_txt[$i])?></p>
					</article>
					<?}}?>
				</div>
			</article>
