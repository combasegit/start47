...
$mode="search";
	if($_REQUEST["del"]){
		...
		$del_id = $_REQUEST["del"];
		//del_flag立てる
		$del_sql = "UPDATE mtb_school_event SET del_flag = '1' WHERE school_id = '$school_id' AND event_id = '$del_id' ";
	...
	$k_sql .= "WHERE m.del_flag = '0' AND school_id = '$school_id'";
	if(($_SESSION["key_all_day"]!='1')&&strlen($_SESSION["key_s_date"])&&strlen($_SESSION["key_e_date"])){
	...
	if(!is_array($err)){
		if($mode=="new"){
			$up_sql = "INSERT INTO mtb_school_event (school_id, event_title, event_text, insert_date)
								 VALUES ('$school_id','$edit_event_title','$edit_event_text',now()) ";
		}else{
