...
$mode="search";
	if($_REQUEST["del"]){
		...
		$del_id = $_REQUEST["del"];
		//del_flag立てる
		$del_sql = "UPDATE dtb_school_news SET del_flag = '1' WHERE school_id = '$school_id' AND news_id = '$del_id' ";
	...
	$k_sql = "select *, date_format(news_date, '%Y/%m/%d') AS news_date2 FROM dtb_school_news WHERE del_flag = '0' AND school_id = '$school_id' AND news_date BETWEEN '".$_SESSION["key_s_date"]." 00:00:01' AND '".$_SESSION["key_e_date"]." 23:59:59' ";
	...
	if(!is_array($err)){
		if($mode=="new"){
			$up_sql = "INSERT INTO dtb_school_news (school_id, news_date, news_title, news_text, insert_date)
								 VALUES ('$school_id','$edit_news_date','$edit_news_title','$edit_news_text',now()) ";
		}else{
